...
const emit = defineEmits(['close', 'safety-added']);
...
const handleSubmit = async () => {
  try {
    ...
    await apiService.post(`/ref-data/safety`, {
...
<template>
  <div class="absolute bg-pink-700 shadow-lg p-6 h-full w-full z-50">
    ...
    <h2 class="text-xl font-bold text-white mb-4 text-center">Add Safety</h2>
    <form @submit.prevent="handleSubmit">
      <div class="mb-4">
        <label for="classNameEn" class="block text-white">Safety name (En)</label>
        <input
            type="text"
            id="classNameEn"
            v-model="safetyNameEn"
            required
            class="border border-gray-300 rounded px-3 py-2 w-full"
        />
      </div>
      <div class="mb-4">
        <label for="classNameAr" class="block text-white">Safety name (Ar)</label>
        <input
            type="text"
            id="classNameAr"
            v-model="safetyNameAr"
            required
            class="border border-gray-300 rounded px-3 py-2 w-full"
        />
      </div>
