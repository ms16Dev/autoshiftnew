...
const emit = defineEmits(['close', 'shape-added']);
...
const handleSubmit = async () => {
  try {
    ...
    await apiService.post(`/ref-data/shapes`, {
      id: shapeId.value,
...
<template>
  <div class="absolute bg-pink-700 shadow-lg p-6 h-full w-full z-50">
    ...
    <h2 class="text-xl font-bold text-white mb-4 text-center">Add Shape</h2>
    <form @submit.prevent="handleSubmit">
      ...
      <div class="mb-4">
        <label for="classNameEn" class="block text-white">Shape name (En)</label>
        <input
            type="text"
            id="classNameEn"
            v-model="shapeNameEn"
      ...
      <div class="mb-4">
        <label for="classNameAr" class="block text-white">Shape name (Ar)</label>
        <input
            type="text"
            id="classNameAr"
            v-model="shapeNameAr"
