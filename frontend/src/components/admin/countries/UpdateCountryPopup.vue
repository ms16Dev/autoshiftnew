...
const handleDelete = async () => {
  try {
    // Use PUT for updates (since we're modifying an existing role)
    await apiService.delete(`/ref-data/countries/${props.id}`);
    ...
    <form @submit.prevent="handleSubmit">
      <div class="mb-4">
        ...
        <input
            type="text"
            id="makeNameEn"
            v-model="countryNameEn"
            required
            class="border border-gray-300 rounded px-3 py-2 w-full"
        />
      </div>
      <div class="mb-4">
        <label for="makeNameAr" class="block text-white">Country name (Ar)</label>
        <input
            type="text"
            id="makeNameAr"
            v-model="countryNameAr"
            required
            class="border border-gray-300 rounded px-3 py-2 w-full"
        />
      </div>
